...
def align_spike_single(step_r, step_n0, nspk_edges, nsearch_start=0):
    ...
    if (
        (idr >= len(step_r))
        or step_r[-1] < step_n0 + nspk_edges[0]
        or step_r[0] > step_n0 + nspk_edges[-1]
    ):
...
def compute_spike_resp(nstep_spk_t, nstep_spk_r, nstep_edges):
    ...
    for n, nstep in enumerate(nstep_spk_t):
        num_spk_set[n], n_search = align_spike_single(
            nstep_spk_r, nstep, nstep_edges, nsearch_start=n_search
        )

    return num_spk_set


def convert_spkvec(step_spk, dt, tmax, srate=2000):
    N = int(tmax * srate) + 1
    spkvec = np.zeros(N)
    for nt in step_spk:
        n = int(nt * dt * srate)
