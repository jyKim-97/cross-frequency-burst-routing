...
def concat_data(
    post_data, key_to_rm=["cv"], include_std=False, show_mm_scale=False, norm_mm=True
):
        ...
        for k2 in keys:
            ...
            if len(shape) == 3:
                ...
                for i in range(ndim_sub):
                    ...
                    if include_std:
                        points[n + ndim, :] = np.array(data_var[:, :, i]).flatten(
                            order="C"
                        )
...
def draw_categorical_colorbar(
    n_category, dn=2, ax=None, cax=None, label=None, label_fontsize=None, **kwargs
):
    import matplotlib.pyplot as plt
