...
def draw_spk(
    step_spk, dt=0.01, sequence=None, xl=None, color_ranges=None, colors=None, ms=1
):
    ...
    if sequence is None:
        ...
        if len(sequence) >= N:
            raise ValueError(
                "Length of sequence (%d) exceeds N (%d)" % (len(sequence), N)
            )
...
class SummaryLoader:
    ...
    def _read_data(self):
        ...
        fnames = [
            ...
            if "id" in f and "result" in f and "monitor" not in f
        ]
        ...
        if nums != nums_expect * self.num_overlap:
            print(
                "Expected number of # results and exact file number are different!: %d/%d"
                % (nums, nums_expect * self.num_overlap)
            )
    ...
    def print_params(self, *nid):
        ...
        for n in range(len(nid)):
            if nid[n] >= self.num_controls[n]:
                print(
                    "Wrong index number typed, size", self.num_controls[n], "typed", nid
                )
...
def imshow_xy(im, x=None, y=None, **kwargs):
    ...
    if x is not None:
        if len(x) != im.shape[1]:
            raise ValueError(
                "len(x)=%d and im.shape[1]=%d do not match" % (len(x), im.shape[1])
            )
    ...
    if y is not None:
        if len(y) != im.shape[0]:
            raise ValueError(
                "len(y)=%d and im.shape[0]=%d do not match" % (len(y), im.shape[0])
            )
