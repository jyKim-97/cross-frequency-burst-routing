...
class SLHC:
    ...
    def __init__(self, metric="precomputed", method="complete"):
        ...
        self.model = AgglomerativeClustering(
            n_clusters=None, metric=metric, linkage=method, distance_threshold=0
        )
    ...
    def cut_dend(self, dth=None, N=None):
        if dth is None and N is not None:
            if N < 2 or N > self.N - 1:
                raise ValueError(
                    "N must be larger than 1 and smaller than the # of data points (%d) - 1"
                    % (self.N)
                )
...
def sort_matrix(dist_mat, linkage_mat):
    ...
    def _seriation(N, linkage_data, cur_index):
        if cur_index < N:
            ...
            return _seriation(N, linkage_data, left) + _seriation(
                N, linkage_data, right
            )
...
def get_silhouette_scores(data, cluster_labels):
    ...
    silhouette_avg = [
        np.average(silhouette_scores[cluster_labels == nt]) for nt in cluster_types
    ]
...
def draw_silhouette(sval, cluster_labels, scoeff=None, cmap="jet"):
    ...
    for n, nt in enumerate(cluster_types):
        ...
        plt.barh(
            id_bar, sval_sub, color=palette(n / num_cluster_types), label="cid%d" % (nt)
        )
        n0 += np.sum(nid)
    ...
    if scoeff is not None:
        plt.plot([scoeff] * 2, yl, "r--", lw=1.0, alpha=0.8, label=r"$s_{coef}$")
...
def draw_with_dendrogram(
    res_linkage, cmat, cluster_id, label=None, color_threshold=None, fig=None
):
    from scipy.cluster.hierarchy import dendrogram
    ...
    plt.imshow(
        cluster_id.reshape((1, -1)), aspect="auto", cmap="Set3", interpolation="none"
    )
...
def realign_cluster(
    cluster_id, col_names, num_r=2, num_w=7, ld=15, denoise=True, nth_remain=3
):
    ...
    for nr in range(num_r):
        for nw in range(num_w):
            ...
            sq_cid, id_col = construct_square_image(
                prefix, cluster_id, col_names, ld=ld
            )
...
def draw_quadratic_summary(
    ...
    hhtools.draw_spk(
        data["step_spk"], color_ranges=cr, colors=cs, xl=xl_raster, sequence=seq
    )
    ...
    for nid in range(3):
        ...
        psd, ff, tf = hhsignal.get_stfft(
            data["vlfp"][nid], data["ts"], 2000, frange=[2, 100]
        )
...
def show_sample_cases(
    ...
    id_sort = np.argsort(silhouette_vals[in_target])
    ...
    for i, tag in enumerate(tags):
        ...
        if save:
            fname = os.path.join(
                fdir, "cid%d_%s(%d).png" % (target_cluster_id, case, i)
            )
        ...
        for j in range(nb):
            for d in ((-1, 0), (1, 0), (0, -1), (0, 1)):
                ...
                if im[i, j] != im[r, c]:
                    if d[1] == 0:
                        lines.append(
                            [[j - 0.5, j + 0.5], [i + 0.5 * d[0], i + 0.5 * d[0]]]
                        )
                    else:
                        lines.append(
                            [[j + 0.5 * d[1], j + 0.5 * d[1]], [i - 0.5, i + 0.5]]
                        )
    return lines


def show_cluster_summary(
    cluster_id, wsets=None, max_cid=15, min_cid=1, sval=None, sth=0.2, figsize=(20, 4)
):
    """
    ...
    for idp in range(num_w):
        ...
        for idr in range(num_r):
            ax = plt.axes(
                position=[0.025 + len_p * idp, 0.025 + (2 - idr) * len_r, len_p, len_r]
            )
            ax_sets[-1].append(ax)
