...
_clib = ctypes.cdll.LoadLibrary(
    "/home/jungyoung/Project/hh_neuralnet/include/estimate_prob.so"
)
...
def estimate_hist_prob_concated(x, y, class_id, nbin=30, nlag_max=20):
    ...
    for _ in range(len(x) // nbuffer + 1):
        ...
        prob3 += _estimate_hist_prob_concated(
            x[n_prv[:n_use]], y[n_prv[:n_use]], class_id[n_prv[:n_use]], nbin, nlag_max
        )
        n_prv = n_use
...
def compute_te(xd, yd, num_bin, nlag_max):
    ...
    if xd.max() >= num_bin or yd.max() >= num_bin:
        raise ValueError(
            "Unexpect bin number in xd : %d, yd: %d" % (xd.max(), yd.max())
        )
