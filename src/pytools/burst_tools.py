...
def draw_binarize_psd(
    psd, pth, x=None, y=None, flim=None, ylabel=None, xlabel=None, axs=None
):
...
def find_blob_filtration(
    psd, psd_th_m, psd_th_s, std_min=3.3, std_max=10, std_step=0.1, nmin_width=3
):
    ...
    for ns in range(len(std_ratio_set)):
        ...
        im_binary = psd >= pth
        expand_null(im_binary, null_map)
...
def explore_new_clusters(im_binary, null_map, burst_map):
    ...
    for nr in range(num_row):
        for nc in range(num_col):
            if (
                im_binary[nr, nc] == 1
                and null_map[nr, nc] == 0
                and burst_map[nr, nc] == 0
            ):
                flag = search_blob(nr, nc, cid, im_class, im_binary)
                if flag == 0:
                    cid += 1
...
def remove_short_burst(bprops):
    ...
    for npop in range(2):
        ...
        for n, (r, f) in enumerate(
            zip(bprops[npop]["burst_range"], bprops[npop]["burst_f"])
        ):
            if r[1] - r[0] < 3 / f / dt:
                continue
...
@jit(nopython=True, cache=True)
def _concatentate_burst_single(
    burst_range: List,
    burst_f: List,
    burst_amp: List,
    id_trial: List,
