...
class AnchoredHScaleBar(matplotlib.offsetbox.AnchoredOffsetbox):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        vline1 = Line2D([0, 0], [-extent / 2.0, extent / 2.0], color=linecolor)
        vline2 = Line2D([size, size], [-extent / 2.0, extent / 2.0], color=linecolor)
        vline3 = Line2D(
            [size / 2, size / 2], [-extent / 4.0, extent / 4.0], color=linecolor
        )
        ...
        txt = matplotlib.offsetbox.TextArea(
            label, textprops=dict(color=fontcolor, size=fontsize, style=fontstyle)
        )
...
def add_scalebar(barsize, label, loc="lower left", ax=None, **kwargs):
    ...
    ob = AnchoredHScaleBar(
        ax=ax, size=barsize, label=label, loc=loc, pad=0.08, sep=2, **kwargs
    )
    ax.add_artist(ob)


def show_psd_summary(
    detail_data,
    frange=(10, 90),
    xrange=(3, 7),
    mbin_t=0.1,
    ...
    for ntp in range(3):
        ...
        psd_t, fpsd, tpsd = hhsignal.get_stfft(
            v, t, fs=fs, mbin_t=mbin_t, wbin_t=wbin_t, frange=frange
        )
