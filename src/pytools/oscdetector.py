...
def detect_osc_motif(
    detail_data: dict, amp_range: dict, mbin_t=0.01, wbin_t=0.5, srate=2000
):

    psd, fpsd, tpsd = compute_stfft_all(
        detail_data, mbin_t=mbin_t, wbin_t=wbin_t, srate=srate
    )
...
def compute_stfft_all(data, frange=(5, 100), mbin_t=0.01, wbin_t=0.5, srate=2000):
    ...
    for v in data["vlfp"]:
        ...
        psd, fpsd, tpsd = hhsignal.get_stfft(
            veq, teq, srate, mbin_t=mbin_t, wbin_t=wbin_t, frange=frange
        )
        psd_set.append(psd)
...
def compute_osc_trit(psd_set, fpsd, tpsd, amp_range, q=80, min_len=2, cat_th=2):
    ...
    for tp in range(2):
        ...
        for nf, ar in enumerate(amp_range[tp_label]):
            ...
            bd_idy_n, _ = pick_osc(
                y, min_len=min_len, cat_th=cat_th, q=100 - q, reversed=True
            )
            for bd in bd_idy_n:
...
def compute_osc_bit(
    psd_set, fpsd, tpsd, amp_range, q=80, min_len=2, cat_th=2, reversed=False
):
    ...
    for tp in range(2):
        ...
        for nf, ar in enumerate(amp_range[tp_label]):
            ...
            y = psd[(fpsd >= ar[0]) & (fpsd < ar[1]), :].mean(axis=0)
            bd_idy, _ = pick_osc(
                y, min_len=min_len, cat_th=cat_th, q=q, reversed=reversed
            )
...
def get_motif_labels(form="ver1"):
    lb = []
    if form == "ver1":
        lb_f, lb_s = ["_", "f"], ["_", "s"]
        par = ["(", ")"]
...
def argsort_motif_labels():
    ...
    return [0, 6, 4, 5, 7, 2, 10, 14, 15, 9, 8, 12, 11, 1, 3, 13]
