...
def show_te2d_summary(
    te_data_2d, figsize=(6, 3), cmap="turbo", dpi=120, vmax=None, vmin=None
):

    te = te_data_2d["te"].mean(axis=0)
    lb_labels = ("F", "S")
    ...
    for ntp in range(2):
        ...
        plt.title(
            r"$TE_{%s \rightarrow %s}$" % (lb_labels[ntp], lb_labels[1 - ntp]),
            fontsize=14,
        )
    ...
    if stat_test:
        ...
        for ntp in range(2):
            for nd in range(xy.shape[-1]):
                is_sig[ntp, nd] = conf_test(
                    xy[:, ntp, nd], xy_s[:, ntp, nd], alpha=0.05
                )
...
def show_spec_summary(
    ...
    _draw_err(
        spec_data["fpsd"], spec_data["spec_boot"][:, 0, :], c=cs[0], label=r"$A_{F}$"
    )
    _draw_err(
        spec_data["fpsd"], spec_data["spec_boot"][:, 1, :], c=cs[1], label=r"$A_{S}$"
    )
...
def show_te_summary_2d(te_data, tl=None, vmax=None, vmin=None, vdmax=None):
    ...
    dte = [te[0] - te_surr[0], (te[1] - te_surr[1]).T]
...
def draw_indicator(
    ...
    if txt is not None:
        ...
        plt.text(
            xind, htxt, txt, va="center", ha="center", fontsize=fontsize, color=color
        )
    plt.ylim(yl)
...
def draw_cfc_indicator(cid, yl=None, flip=False, h=None):
    ...
    for dt in dt_set:
        ...
        draw_indicator(
            dt, yl, color=c, linestyle="-.", txt=r"$T^{CFC}$", flip=flip, htxt=h
        )


def draw_freq_indicator(
    cid=None, yl=None, flip=False, h=None, lw=1, alpha=1, f0_set=None
):
    ...
    if f0_set is None:
        if cid is None:
            raise ValueError("cid or f0_set need to be determined")
        f0_set = fpeaks[cid - 1]
    ...
    for tp in range(2):
        ...
        draw_indicator(
            1e3 / f0 / 2, yl, color=c, txt=tp_labels[2 * tp], htxt=h, flip=flip, **lopt
        )
        draw_indicator(
            1e3 / f0, yl, color=c, txt=tp_labels[2 * tp + 1], htxt=h, flip=flip, **lopt
        )
...
def draw_syn_indicator(yl=None, flip=None, h=None):
    ...
    tp_set = [
        tau1[n] * tau2[n] / (tau2[n] - tau1[n]) * np.log(tau2[n] / tau1[n])
        for n in range(3)
    ]
    labels = (r"$t^*_{E}$", r"$t^*_{I_F}$", r"$t^*_{I_S}$")
...
def draw_barcode(
    ...
    plt.imshow(
        binfo["barcode"], aspect="auto", cmap=cmap, extent=extent, vmax=vmax, vmin=-vmax
    )
...
def draw_arrow_axis(
    xmax=30, h=1, width=0.02, head_length=1, head_width=0.05, fontsize=4
):
    ax = plt.gca()
    for n in range(2):
    ...
    plt.text(
        0, h + w, r"$\tau=0$", fontsize=fontsize, color="k", ha="center", va="bottom"
    )
    plt.text(
        xmax,
        h + w,
...
def draw_round_rect(
    ...
    for ts in tsets:
        ...
        polygon = patches.Polygon(
            points, closed=True, fill=True, color=color, lw=lw, alpha=alpha
        )
        ax.add_patch(polygon)


def draw_arrow_line(
    tsets: List, y=0, dy=30, width=0.02, head_length=1, head_width=0.05, color="k"
):
    ax = plt.gca()
    for ts in tsets:
        x0 = (ts[0] + ts[1]) / 2
...
def draw_te_diagram_full(
    tsig_sets, xmax=30, y0=30, colors_arrow=None, colors_rect=None, fontsize=4
):
    ...
    draw_arrow_axis(
        xmax=xmax, h=y0, head_length=2, width=0.5, head_width=2, fontsize=fontsize
    )
...
def draw_reduce_axis(
    y=30, xmax=30, head_length=2, width=0.5, fs=6, lw=1, lb_text=("F", "S")
):
    ax = plt.gca()
    for n in range(2):
        h0 = y * n
...
def draw_te_diagram_reduce(
    ...
    assert visu_type in (
        ...
        "arrow",
        "arrow_only",
    ), "visu_type must be 'box' or 'arrow'"
    ...
    if visu_type == "box":
        ...
        draw_round_rect(
            ...
            alpha=alpha,
            show_arrow=True,
            arrow_h=-box_height * arrow_ratio,
        )
        draw_round_rect(
            tsig_sets[1],
            xmax=xmax,
