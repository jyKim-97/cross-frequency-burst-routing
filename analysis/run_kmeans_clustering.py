...
def build_parser():
    ...
    parser.add_argument(
        "--kmin", default=3, help="minimum number of clusters", type=int
    )
    parser.add_argument(
        "--kmax", default=15, help="maximum number of clusters", type=int
    )
    parser.add_argument(
        "--nitr", default=100, help="The number of iterations", type=int
    )
    return parser
...
def main(fdata=None, fout=None, kmin=3, kmax=15, nitr=100):
    ...
    for n in tqdm(range(len(ksets) * nitr), desc="kmeans clustering"):
        ...
        meta_info.append({"k": k, "seed": seed})

        kobj = cluster.KMeans(
            n_clusters=k, n_init=1, copy_x=True, random_state=seed, init="k-means++"
        )
        pred_id = kobj.fit_predict(data.T)
        ks = kobj.score(data.T)
        ss = silhouette_score(
            data.T.copy(), pred_id, sample_size=1000, metric="euclidean"
        )
