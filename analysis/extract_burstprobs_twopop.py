...
fdir = "./data"
famp_range = "../mfop_results/mfop/amp_range_set.pkl"
wbin_t = 0.5
mbin_t = 0.01
max_std_ratio = 5
min_std_ratio = 1.96
...
def build_parser():
    ...
    parser.add_argument(
        "--fout", type=str, default="./tmp.pkl", help="File name to save the data"
    )
...
def extract_probs(summary_obj, cid):
    ...
    burst_props = [
        dict(
            burst_f=[], burst_len=[], burst_amp=[], burst_range=[], id_trial=[], tpsd=[]
        )
        for _ in range(2)
    ]
    ...
    for nt in tqdm(range(num_trial)):
        ...
        for k in range(2):
            ...
            bmap = bt.find_blob_filtration(
                ...
                th_m,
                th_s,
                std_min=min_std_ratio,
                std_max=max_std_ratio,
                std_step=0.1,
                nmin_width=1,
            )
