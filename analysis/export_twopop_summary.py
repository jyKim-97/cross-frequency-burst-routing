...
def build_arg_parser():
    ...
    parser.add_argument(
        "--fout", help="output file name", default="./dynamic_orders.nc"
    )
...
def pick_sample_data(data):
    ...
    vlfp = [data["vlfp"][0][n0:n1], data["vlfp"][1][n0:n1], data["vlfp"][2][n0:n1]]
    return vlfp, t0
...
def get_cc_peak(x, y, prominence=0.01):
    ...
    idp3 = idp[np.argsort(np.abs(tlag[idp]))[:3]]
...
def extract_single_result(sample):
    ...
    data = sample.load()
    res = np.zeros([len(keys_dyna), 3, 2])

    tau_ac_peaks = np.zeros([num_itr, 2])
    ...
    for nid in range(num_itr):
        ...
        for tp in range(3):
            vals_ac2 = get_ac2_peak(vlfp[tp], prominence=prominence)
            vals_ft = get_fft_peak(vlfp[tp])
...
def construct_args(summary_obj, desc=None):
    ...
    dataset = parrun(
        _load_single, np.arange(summary_obj.num_total, dtype=int), desc=desc
    )
...
def align_order(summary_obj):
    ...
    for i, key in enumerate(keys_order):
        for nf, func in enumerate((np.nanmean, np.nanstd)):
            tmp = func(summary_obj.summary[key], axis=4)
            for tp in range(3):
...
def extract_result(summary_obj) -> xr.DataArray:
    ...
    res_dyna = np.zeros([len(keys_dyna), 3, 2, nums])

    for n in range(nums):
        res_dyna[:, :, :, n] = np.average(
            res_set[n * nt : (n + 1) * nt, :, :, :], axis=0
        )
    res_dyna[:-1, :, 1, :] = res_dyna[:-1, :, 1, :] - res_dyna[:-1, :, 0, :] ** 2
